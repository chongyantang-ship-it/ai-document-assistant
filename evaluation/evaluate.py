...
def judge_answer(result, ground_truth_row):
    ...
    evidence_text = " ".join(
        str(ev.get("text", "")) for ev in result.get("evidence", [])
        if isinstance(ev, dict)
    )

    if answerable:
        answer_correct = int(
            answer_contains_required_keywords(answer_text, required_keywords)
            or answer_contains_required_keywords(evidence_text, required_keywords)
        )
